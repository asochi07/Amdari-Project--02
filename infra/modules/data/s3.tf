###############################################################################
# S3 - application data bucket (e.g. KYC documents). Customer-managed KMS
# encryption, fully public-access-blocked, versioned, TLS-only bucket policy.
# This replaces the original public-read KYC bucket anti-pattern.
###############################################################################

resource "aws_s3_bucket" "app" {
  bucket = "${var.name_prefix}-app-${local.account_id}"
